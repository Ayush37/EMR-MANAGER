...
### Planned Enhancement: Portal Access Control & User Management

#### Current Cookie Implementation
- **Cookie Name**: `dxd-ct-auth` (ADFS authentication cookie)
- **Current Usage**: Displays user's firstName in header ("Hello, {firstName}")
- **Location**: home-frontend/client/src/App.js (lines 93-113)
- **Data Extraction**: Cookie is URL-decoded and parsed as JSON to get user info

#### Planned Access Control System
1. **Authentication Gate**: 
   - Block portal access without valid ADFS cookie
   - Redirect to ADFS login or show access denied page
   
2. **DynamoDB User Allowlist**:
   - New backend service: `/portal-api/*`
   - Table: `aws-xcess-users`
     - userId (PK): email from cookie
     - accessLevel: 'full', 'read-only', 'viewer'
     - services: ['emr', 'ssm', 's3data'] - service-specific access
     - expiryDate: optional access expiration
     - department, addedBy, addedDate
   
3. **User Analytics**:
   - Track unique users, service usage patterns
   - Store in DynamoDB or CloudWatch metrics
   
4. **User Preferences**:
   - Store dark mode, sidebar state, favorite services
   - Link to userId from cookie
   
5. **Portal API Endpoints**:
   - `/portal-api/verify-access` - Check user allowlist
   - `/portal-api/preferences` - Get/update preferences
   - `/portal-api/track-usage` - Analytics logging
